...
def detect(input_dir, output_dir, crop_dim):
    ...
    image_output_dir = os.path.join(output_dir, os.path.basename(input_dir))
    ...
    logger.info('Preprocess Completed in {} seconds'.format(time.time() - start_time))
...
def init(dirname):
    ...
    parser.add_argument('--output-dir', type=str, action='store', default='processed_training_data', dest='output_dir')
    ...
    detect(input_dir, args.output_dir, args.crop_dim)
